...
void main() {
    ...
    float zFrom = fromProjected.z / fromProjected.w;
    float zTo = toProjected.z / toProjected.w;

    vec2 p0 = fromScreen - dirScreen - perpScreen;
    gl_Position = vec4(p0 / viewportSize, zFrom, 1.0);
    gs_texCoord = vec2(0.0, 0.0);
    EmitVertex();

    vec2 p1 = p0 + 2.0 * perpScreen;
    gl_Position = vec4(p1 / viewportSize, zFrom, 1.0);
    gs_texCoord = vec2(0.0, 1.0);
    EmitVertex();

    vec2 p2 = p0 + dirScreen;
    gl_Position = vec4(p2 / viewportSize, zFrom, 1.0);
    gs_texCoord = vec2(0.5, 0.0);
    EmitVertex();

    vec2 p3 = p2 + 2.0 * perpScreen;
    gl_Position = vec4(p3 / viewportSize, zFrom, 1.0);
    gs_texCoord = vec2(0.5, 1.0);
    EmitVertex();

    vec2 p4 = toScreen - perpScreen;
    gl_Position = vec4(p4 / viewportSize, zTo, 1.0);
    gs_texCoord = vec2(0.5, 0.0);
    EmitVertex();

    vec2 p5 = p4 + 2.0 * perpScreen;
    gl_Position = vec4(p5 / viewportSize, zTo, 1.0);
    gs_texCoord = vec2(0.5, 1.0);
    EmitVertex();

    vec2 p6 = p4 + dirScreen;
    gl_Position = vec4(p6 / viewportSize, zTo, 1.0);
    gs_texCoord = vec2(1.0, 0.0);
    EmitVertex();

    vec2 p7 = p6 + 2.0 * perpScreen;
    gl_Position = vec4(p7 / viewportSize, zTo, 1.0);
